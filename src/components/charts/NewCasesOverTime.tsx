...
export default function NewCasesOverTime({ data }: ChartData) {
    ...
    useEffect(() => {
        ...
        const casesA = sortedDataA.map(entry => entry.newCases);
        const casesB = sortedDataB.map(entry => entry.newCases);

        const options = {
            title: {
                text: "Novos Casos x Tempo",
                left: "center",
            },
            tooltip: {
                trigger: "axis",
