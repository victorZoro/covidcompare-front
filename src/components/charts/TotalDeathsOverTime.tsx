...
export default function TotalDeathsOverTime({ data }: ChartData) {
    ...
    useEffect(() => {
        ...
        const deathsA = sortedDataA.map(entry => entry.totalDeaths);
        const deathsB = sortedDataB.map(entry => entry.totalDeaths);

        const options = {
            title: {
                text: "Mortes Totais x Tempo",
                left: "center",
            },
            tooltip: {
                trigger: "axis",
